...
def remove_silence(filename, out_dir, smoothing=1.0, weight=0.3, plot=False):
    ...
    if is_segmentable(participant_id):
        ...
        segments = aS.silence_removal(
            x, fs, 0.02, 0.02, smooth_window=smoothing, weight=weight, plot=plot
        )

        for s in segments:
            seg_name = "{:s}_{:.2f}-{:.2f}.wav".format(participant_id, s[0], s[1])
            wavfile.write(seg_name, fs, x[int(fs * s[0]) : int(fs * s[1])])

        # concatenate segmented wave files within participant directory
        concatenate_segments(participant_dir, participant_id)


def is_segmentable(participant_id):
    """
...
def concatenate_segments(participant_dir, participant_id, remove_segment=True):
    """
    ...
    outfile = "{}_no_silence.wav".format(participant_id)
