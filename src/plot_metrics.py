...
def plot_accuracy(history, model_id):
    ...
    plt.savefig(
        os.path.join(config.BASE_DIR, "images", "cnn{}_accuracy.png".format(model_id))
    )
    plt.close()
...
def plot_loss(history, model_id):
    ...
    plt.legend(["train", "test"], loc="upper left")
    plt.savefig(
        os.path.join(config.BASE_DIR, "images", "cnn{}_loss.png".format(model_id))
    )
    plt.close()
...
def plot_roc_curve(y_test, y_score, model_id):
    ...
    plt.savefig(
        os.path.join(config.BASE_DIR, "images", "cnn{}_roc.png".format(model_id))
    )
    plt.close()
